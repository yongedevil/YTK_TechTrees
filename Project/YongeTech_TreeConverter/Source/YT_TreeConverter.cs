...
namespace YongeTechKerbal
{
    ...
    public class YT_TreeConverter : MonoBehaviour
    {
        ...
        {
            Log.Info("YT_TreeConverter.CompleteTree()");
            ...
            foreach (ConfigNode RDNode in completedTreeNode.GetNodes())
            {
                ...
                {
                    Log.Info("YT_TreeConverter.CompleteTree(): ERROR techID not found for RDNode. Node:\n" + RDNode.ToString());
                    continue;
                }
            ...
            try
            {
                Log.Info("YT_TreeConverter.ConvertTree(): writing tech tree data to: " + UrlDir.CreateApplicationPath(YT_TreeConverterSettings.Instance.TreeWriteDir + fileName));
            ...
            {
                Log.Info("YT_TreeConverter.ConvertTree(): ERROR path too long exception");
                writeFailure = true;
            }
            catch(IsolatedStorageException)
            {
                Log.Info("YT_TreeConverter.ConvertTree(): ERROR IsolatedStorageException");
                writeFailure = true;
            }
            ...
            if(writeFailure)
            {
                Log.Info("YT_TreeConverter.ConvertTree(): Printing converted tree to log:\n" + completedTreeNode.ToString());
            }
        ...
        {
            Log.Info("YT_TreeConverter.GetPartsContainedInTree()");
        ...
        {
            Log.Info("YT_TreeConverter.GetPartsWithTechRequired()");
